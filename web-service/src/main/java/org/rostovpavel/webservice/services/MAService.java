...
import org.rostovpavel.base.models.MA.*;
...
public class MAService {
    private final static int[] lengthMA = new int[]{20, 50, 100};

    public MovingAverage getMovingAverage(StockDTO date){
        List<Stock> stocks = date.getStocks();
        EMA emaRes = EMA.builder().build();
        SMA smaRes = SMA.builder().build();

        Arrays.stream(lengthMA)
                .forEach(len -> {
    ...
    private BigDecimal getSMA (List<Stock> stocks, int length){
        return stocks
                ...
                .divide(BigDecimal.valueOf(length));
    }
